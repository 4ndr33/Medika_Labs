...
function bindUserTableButtons(table) {
    ...
    $(table).find('.js-user-offer-user-admin').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        OfferUserAdmin(user_id)
        .always(function(response) {
            // Reload page after updating user details
            window.location.reload();
        });
    ...
    $(table).find('.js-user-offer-provider-admin').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        OfferProviderAdmin(user_id)
        .always(function(response) {
            // Reload page after updating user details
            window.location.reload();
        });
    ...
    $(table).find('.js-user-accept-offer-provider').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        AcceptOfferProvider(user_id)
        .always(function(response) {
            // Reload page after updating user details
            window.location.reload();
        });
    ...
    $(table).find('.js-user-reject-offer-provider').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        RejectOfferProvider(user_id)
        .always(function(response) {
            // Reload page after updating user details
            window.location.reload();
        });
	...
	$(table).find('.js-ticket-delete').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        var user_name = btn.data('id');
        deleteTicketDialog('dialog-ticket-delete', user_id, user_name);
    });	
}
...
function OfferUserAdmin(user_id) {
	csrf_token = $("meta[name=csrf_token]").attr("content");
    var data = {
		flag_verified: "1",
        csrf_token: csrf_token
    ...
    var url = site['uri']['public'] + "/offer-user-admin/" + user_id;
...
function OfferProviderAdmin(user_id) {
	csrf_token = $("meta[name=csrf_token]").attr("content");
    var data = {
		flag_verified: "1",
        csrf_token: csrf_token
    ...
    var url = site['uri']['public'] + "/offer-provider-admin/" + user_id;
...
function AcceptOfferProvider(user_id) {
	csrf_token = $("meta[name=csrf_token]").attr("content");
    var data = {
		flag_verified: "1",
        csrf_token: csrf_token
    ...
    var url = site['uri']['public'] + "/accept-offer-provider/" + user_id;
...
function RejectOfferProvider(user_id) {
	csrf_token = $("meta[name=csrf_token]").attr("content");
    var data = {
		flag_verified: "1",
        csrf_token: csrf_token
    ...
    var url = site['uri']['public'] + "/reject-offer-provider/" + user_id;
...
function deleteTicketDialog(box_id, user_id, name){
	// Delete any existing instance of the form with the same name
	if($('#' + box_id).length ) {
		$('#' + box_id).remove();
	}
	...
	var data = {
		box_id: box_id,
		box_title: "Delete Coupon",
		confirm_message: "Are you sure you want to delete the coupon " + name + "?",
		confirm_button: "Yes, delete coupon"
	};
	...
	.done(function(result) {		
		...
		$('#' + box_id + ' .js-confirm').click(function(){
            ...
            var url = site['uri']['public'] + "/tickets/u/" + user_id + "/delete";
