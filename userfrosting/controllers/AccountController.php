     ...
     */
	 public function setPassword($flag_new_user = false){
        ...
        if ($user->flag_password_reset == 0 || $last_password_reset_time === null){
            $ms->addMessageTranslated("danger", "FORGOTPASS_INVALID_TOKEN");
        	//$ms->addMessageTranslated("danger", $user->flag_password_reset);
            $this->_app->halt(400);
        }
    	...
    	$current_time2 = date("Y-m-d H-i-s",strtotime('+13 hour'));
		...
		$current_token_life = strtotime('+13 hour') - $last_password_reset_datetime->getTimestamp();
        ...
        if($current_token_life >= $expiration|| $current_token_life < 0){
        	...
        	//$ms->addMessageTranslated("danger","".$current_time2. $last_password_reset_time);
